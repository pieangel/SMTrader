#pragma once
#include "VtSystem.h"
#include "../Xml/pugixml.hpp"
class VtChartData;
class VtChannelCrossSystem :
	public VtSystem
{
public:
	VtChannelCrossSystem();
	VtChannelCrossSystem(VtSystemType type);
	VtChannelCrossSystem(VtSystemType systemType, std::string name);
	virtual ~VtChannelCrossSystem();

	virtual void InitArgs();
	virtual void CreateSignal(int startIndex, int endIndex);
	virtual VtPositionType UpdateSignal(int index);
	virtual void ReloadSystem(int startIndex, int endIndex);
	virtual VtPositionType UpdateSignal(int start, int end);
	virtual void SetDataMap(VtChartData* chartData);
	int ChannelPeriod;
	int AtrPeriod;
	int AtrMul;
	int BarsSinceE;

	virtual void SaveToXml(pugi::xml_node& node);
	virtual void LoadFromXml(pugi::xml_node& node);
};

