						...
						int selected,int current)
{
	...
	CPoint pt1(rectout.left, rectout.top + (rectout.bottom - rectout.top) / 2);
	CPoint pt2(rectout.right, rectout.top + (rectout.bottom - rectout.top) / 2);
	if (cell->DrawArrow())
	{
		DrawArrow(dc, pt1, pt2, 5, 5);
	}
...
void CUGCellType::DrawArrow(CDC* pdc, POINT p0, POINT p1, int head_length, int head_width)
{
...
void CUGCellType::DrawRemain(CDC* pdc, POINT p)
{
	CBrush brush1;   // Must initialize!
	...
	rcRemain.left = p.x;
	rcRemain.top = p.y;
	rcRemain.right = p.x + 8;
	rcRemain.bottom = p.y + 8;
	pdc->FillRect(&rcRemain, pTempBrush);

	// Reselect original brush into device context.
	pdc->SelectObject(&OrigBrush);
}

void CUGCellType::DrawFocusedRect(CDC *dc, RECT *rect)
{
	CPen pen;
...
int CUGCellType::DrawBitmap(CDC *dc,CBitmap * bitmap,RECT *rect,COLORREF backcolor)
{
	...
	{
		int rectWid = rect->right - rect->left;
		int bitmapWid = bm.bmWidth;
		int margin = rectWid - bitmapWid;
		// Display the bitmap on screen
		if(resize)
		{
			dc->StretchBlt(rect->left+Margin + (margin / 2),rect->top+Margin,(int)xout,(int)yout,
