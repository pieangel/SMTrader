...
int CUGCell::AddCellInfo(CUGCell *src,CUGCell *dest)
{
	...
	// initial state
	dest->m_orderCount = src->m_orderCount;
	dest->m_cellInitialState = src->m_cellInitialState;
	dest->m_DrawRect = src->m_DrawRect;
	dest->m_DrawArrow = src->m_DrawArrow;
	dest->m_Focused = src->m_Focused;
	dest->m_orderType = src->m_orderType;
	dest->m_OrderSlip = src->m_OrderSlip;
	dest->m_Tag = src->m_Tag;
	dest->m_LongValue = src->m_LongValue;
	dest->UserMessage(src->UserMessage());
	dest->m_HasLabel = src->m_HasLabel;
	dest->m_CellLable = src->m_CellLable;
	// Copy all the orders from the source cell to the target cell.
	dest->OrderList.clear();
	for (auto it = src->OrderList.begin(); it != src->OrderList.end(); it++)
	{
		VtOrder* order = *it;
		dest->OrderList.push_back(order);
	}


	dest->OrderMap.clear();
	for (auto it = src->OrderMap.begin(); it != src->OrderMap.end(); ++it)
	{
		VtOrder* order = it->second;
		dest->OrderMap[order->orderNo] = order;
	}

	dest->StopOrderList.clear();
	for (auto it = src->StopOrderList.begin(); it != src->StopOrderList.end(); it++)
	{
		VtOrderRequest* orderReq = *it;
		dest->StopOrderList.push_back(orderReq);
	}

	dest->StopOrderListHd.clear();
	for (auto it = src->StopOrderListHd.begin(); it != src->StopOrderListHd.end(); it++)
	{
		HdOrderRequest* orderReq = *it;
		dest->StopOrderListHd.push_back(orderReq);
	}
