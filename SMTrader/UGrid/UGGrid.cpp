...
CUGGrid::CUGGrid()
{
	m_ArrowFont.CreateFont(10, 0, 0, 0, 900, 0, 0, 0, 0, 0, 0, 0, 0, _T("굴림"));
...
void CUGGrid::OnMouseLeave()
{
	...
	if (m_ctrl)
		m_ctrl->OnMouseLeaveFromMainGrid();
...
void CUGGrid::DrawCellsIntern(CDC *dc,CDC *db_dc)
{
	...
	//m_ctrl->OnDrawSelectedRect(dc, &focusRect);
	DrawSelectedRect(dc, m_ctrl->GetClickRect());
	
	// 스탑 주문 화살표를 표시한다.
	if (m_ctrl->ArrowPosVector.size() > 0)
	{
		for( auto it = m_ctrl->ArrowPosVector.begin(); it != m_ctrl->ArrowPosVector.end(); ++it)
		{
			VtArrowPos pos = *it;
			if (pos.type == 2) // for put
			{
				if (pos.slip >= 0)
				{
					if (pos.row >= pos.startRow && pos.row <= (pos.endRow - pos.slip + 2))
					{
						CPoint pt1(pos.sx, pos.sy);
						CPoint pt2(pos.ex, pos.ey);
						DrawArrow(pos.type, nullptr, dc, pt1, pt2, 6, 6);
					}
				}
			}
			else if (pos.type == 1) // for call
			{
				if (pos.row <= pos.endRow && pos.row >= (pos.slip - pos.startRow))
				{
					CPoint pt1(pos.sx, pos.sy);
					CPoint pt2(pos.ex, pos.ey);
					DrawArrow(pos.type, nullptr, dc, pt1, pt2, 6, 6);
...
void CUGGrid::DrawArrow(int direction, POINT *point, CDC* pdc, POINT p0, POINT p1, int head_length, int head_width)
{
	...
	rc.right = rc.left + 44;
	rc.bottom = rc.top + 22;
	//CFont* oldFont = pdc->SelectObject(&m_ArrowFont);
...
void CUGGrid::DrawSelectedRect(CDC *dc, RECT *rect)
{
	if (rect == nullptr)
		return;
	...
	CPen pen;
	CPen* oldPen = nullptr;
	pen.CreatePen(PS_SOLID, 2, RGB(0, 102, 51));
	oldPen = (CPen*)dc->SelectObject(&pen);

	RECT rectCenter;
	CopyRect(&rectCenter, rect);
...
{
	//CString msg;
	//msg.Format(_T("zDelta row = %d\n"), zDelta);
	//TRACE(msg);
	if (m_ctrl->GetUGMode() == UGMode::Normal)
	{
		// TODO: Add your message handler code here and/or call default
		if (!m_ctrl->m_editInProgress) {
	...
	else if (m_ctrl->GetUGMode() == UGMode::OrderNormal)
	{
		...
		if (abs(zDelta) > 120)
			...
			distance = zDelta / 40;
		long oldCenter = m_ctrl->GetCenterRow();
		long newCenter = m_ctrl->GetCenterRow() + (distance);
		m_ctrl->SetCenterRow(newCenter);
		m_ctrl->OnGridWheel(oldCenter, newCenter);
		m_ctrl->m_CUGSideHdg->Invalidate();
	}
