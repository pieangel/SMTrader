...
{
public:
	UTF8toTCHAREx(const char *utf8_string) : t_string(0), needFree(false)
	{
		if (0 == utf8_string)
			t_string = 0;
...
private:
	...
	//disable assignment
	UTF8toTCHAREx(const UTF8toTCHAREx &rhs);
	UTF8toTCHAREx &operator=(const UTF8toTCHAREx &rhs);
};
...
{
public:
	TCHARtoUTF8Ex(const TCHAR *t_string) : utf8_string(0), needFree(false)
	{
		if (0 == t_string)
			utf8_string = 0;
		else if (0 == *t_string)
...
private:
	...
	//disable assignment
	TCHARtoUTF8Ex(const TCHARtoUTF8Ex &rhs);
	TCHARtoUTF8Ex &operator=(const TCHARtoUTF8Ex &rhs);
};
...
private:
	...
	std::map<std::string, VtRefChart> m_RefereneChartMap;
	...
	std::string m_firstMinFormat;
	std::string m_otherMinFormat;
...
public:
	XYChart* AddReferenceChart(int height, std::string name, int type, DoubleArray high, DoubleArray low, DoubleArray open, DoubleArray close, std::string dataName)
	{
		VtRefChart ref;
		ref.height = height;
		ref.name = name;
		ref.type = type;
		deepCopy(ref.high, high);
		deepCopy(ref.low, low);
		deepCopy(ref.open, open);
		deepCopy(ref.close, close);

		XYChart *c = addIndicator(height);
		int upColor = 0xff0000;
		int downColor = 0x0000ff;

		//addOHLCLabel(upColor, downColor, true);

		CandleStickLayer *ret = c->addCandleStickLayer(ref.high, ref.low, ref.open,
			ref.close, upColor, downColor);
		ret->setColors(0xff0000, 0xff0000, 0x0000ff, 0x0000ff);

		ret->setHTMLImageMap("", "", getHLOCToolTipFormat().c_str());
		if (ref.high.len - m_extraPoints > 60) {
			ret->setDataGap(0);
		}

		if (ref.high.len > m_extraPoints) {
			int expectedWidth = (m_totalWidth - m_leftMargin - m_rightMargin) / (ref.high.len - m_extraPoints);
			if (expectedWidth <= 5)
			{
		...
		DataSet* dataSet = ret->getDataSet(0);
		dataSet->setDataName(dataName.c_str());

		m_RefereneChartMap[name] = ref;

		return c;
	}

	XYChart* AddReferenceChart2(int height, std::string name, int type, int width, int color, Layer** layer, DoubleArray high, DoubleArray low, DoubleArray open, DoubleArray close, std::string dataName)
	{
		VtRefChart ref;
		ref.height = height;
		ref.name = name;
		...
		{
			LineLayer* linelayer = c->addLineLayer(ref.close, color, dataName.c_str());
    ...
    private: void init()
    {
        ...
        m_firstMonthFormat = "<*font=bold*>{value|yy-mm}";
        ...
        m_toolTipMonthFormat = "[{xLabel|mm, yyyy}]";
        m_toolTipDayFormat = "[{xLabel|mm/dd, yyyy}]";
        m_toolTipHourFormat = "[{xLabel|mm/dd, yyyy hh:nn:ss}]";
    ...
    public: ~FinanceChart()
    {
		...
		for (auto it = m_RefereneChartMap.begin(); it != m_RefereneChartMap.end(); ++it)
		{
			VtRefChart curData = it->second;
			delete[] const_cast<double *>(curData.high.data);
			delete[] const_cast<double *>(curData.low.data);
			delete[] const_cast<double *>(curData.open.data);
			delete[] const_cast<double *>(curData.close.data);
		}
    ...
    public: CandleStickLayer* addCandleStick(int upColor, int downColor)
    {
        ...
        if (m_highData.len > m_extraPoints) {
			...
			m_BarWidth = expectedWidth;
			if (expectedWidth <= 5)
			{
				ret->setDataWidth(expectedWidth + 1 - expectedWidth % 2);
				m_BarWidth = expectedWidth + 1 - expectedWidth % 2;
			}
        }

		DataSet* dataSet = ret->getDataSet(0);
		dataSet->setDataName(_T("MainChart;"));
        return ret;
    }
			...
			/// <param name="downColor">The candle color for a down day.</param>
			/// <returns>The CandleStickLayer created.</returns>
	public: CandleStickLayer* addCandleStick(int upColor, int downColor, const char* info)
	{
		addOHLCLabel(upColor, downColor, true);
		CandleStickLayer *ret = m_mainChart->addCandleStickLayer(m_highData, m_lowData, m_openData,
			m_closeData, upColor, downColor);
		...
		DataSet* dataSet = ret->getDataSet(0);
		dataSet->setDataName(info);
		return ret;
	}

	 public: CandleStickLayer* addCandleStick2(int upColor, int downColor, std::string dataName)
	 {
		 addOHLCLabel(upColor, downColor, true);
		 CandleStickLayer *ret = m_mainChart->addCandleStickLayer(m_highData, m_lowData, m_openData,
			 m_closeData, upColor, downColor);
		 ...
		 dataName.append(_T(";"));
		 DataSet* dataSet = ret->getDataSet(0);
		 dataSet->setDataName(datasetName.c_str());
		 return ret;
	 }
    ...
    public: HLOCLayer* addHLOC(int upColor, int downColor)
    {
        ...
        return ret;
			...
			/// <param name="downColor">The color of the HLOC symbol for a down day.</param>
			/// <returns>The HLOCLayer created.</returns>
	public: HLOCLayer* addHLOC(int upColor, int downColor, const char* info)
	{
		addOHLCLabel(upColor, downColor, false);
		HLOCLayer *ret = m_mainChart->addHLOCLayer(m_highData, m_lowData, m_openData, m_closeData);
		ret->setColorMethod(Chart::HLOCUpDown, upColor, downColor);
    ...
    {
		return addLineIndicator2(m_mainChart, m_closeData, color, "MainChart;Closing Price");
    }
	...
	/// <param name="color">The color of the line.</param>
	/// <returns>The LineLayer object representing the line created.</returns>
	public: LineLayer* addCloseLine(int color, const char* name)
	{
		return addLineIndicator2(m_mainChart, m_closeData, color, name);
	}
    ...
    public: LineLayer* addWeightedClose(int color)
    {
        return addLineIndicator2(m_mainChart, ArrayMath(m_highData).add(m_lowData).add(m_closeData
            ).add(m_closeData).div(4), color, "MainChart;Weighted Close");
    }
			...
			/// <param name="color">The color of the line.</param>
			/// <returns>The LineLayer object representing the line created.</returns>
	public: LineLayer* addWeightedClose(int color, const char* info)
	{
		return addLineIndicator2(m_mainChart, ArrayMath(m_highData).add(m_lowData).add(m_closeData
		).add(m_closeData).div(4), color, info);
	}
    ...
    {
        return addLineIndicator2(m_mainChart, ArrayMath(m_highData).add(m_lowData).add(m_closeData
            ).div(3), color, "MainChart;Typical Price");
    }
			...
			/// <param name="color">The color of the line.</param>
			/// <returns>The LineLayer object representing the line created.</returns>
	public: LineLayer* addTypicalPrice(int color, const char* info)
	{
		return addLineIndicator2(m_mainChart, ArrayMath(m_highData).add(m_lowData).add(m_closeData
		).div(3), color, info);
	}
    ...
    public: LineLayer* addMedianPrice(int color)
    {
        return addLineIndicator2(m_mainChart, ArrayMath(m_highData).add(m_lowData).div(2), color,
			...
			/// <param name="color">The color of the line.</param>
			/// <returns>The LineLayer object representing the line created.</returns>
	public: LineLayer* addMedianPrice(int color, const char* info)
	{
		return addLineIndicator2(m_mainChart, ArrayMath(m_highData).add(m_lowData).div(2), color, info);
	}
	...
	public: LineLayer* addSimpleMovingAvg2(std::string alias, int period, int color)
	{
		char buffer[1024];
		sprintf(buffer, "%s;SMA (%d)", alias.c_str(), period);
		return addLineIndicator2(m_mainChart, ArrayMath(m_closeData).movAvg(period), color, buffer);
	}
        ...
        int fillColor, const char* name)
    {
		...
		sprintf(buffer, "%s:%d", tempName, 1);
        layer->addDataSet(upperLine, lineColor, buffer);
		sprintf(buffer, "%s:%d", tempName, 2);
        layer->addDataSet(lowerLine, lineColor, buffer);
        return m_mainChart->addInterLineLayer(layer->getLine(0), layer->getLine(1), fillColor);
    }

	 public: InterLineLayer* addBand2(DoubleArray upperLine, DoubleArray lowerLine, int highColor, int lowColor,
		 int fillColor, const char* name)
	 {
		 int i = upperLine.len - 1;
		 if (i >= lowerLine.len) {
		 ...
		 while (i >= 0) {
			 if ((upperLine[i] != Chart::NoValue) && (lowerLine[i] != Chart::NoValue)) {
				 ...
				 //name = strcat(buffer, formatValue(upperLine[i], m_generalFormat.c_str()));
				 sprintf(buffer, "%s: %s", name, formatValue(lowerLine[i], m_generalFormat.c_str()));
				 sprintf(buffer2, "%s: %s", name, formatValue(upperLine[i], m_generalFormat.c_str()));
				 break;
			 }
			 i = i - 1;
		 }
		 ...
		 memset(buffer, 0x00, 1024);
		 //sprintf(buffer, "%s:%d", tempName, 1);
		 layer->addDataSet(upperLine, highColor, buffer2);
		 //sprintf(buffer2, "%s:%d", tempName, 2);
		 layer->addDataSet(lowerLine, lowColor, buffer);
		 return m_mainChart->addInterLineLayer(layer->getLine(0), layer->getLine(1), fillColor);
	 }
	...
	public: InterLineLayer* addBollingerBand2(std::string alias, int period, double bandWidth, int lineColor,
		int fillColor)
	{
		//Bollinger Band is moving avg +/- (width * moving std deviation)
		ArrayMath stdDev = ArrayMath(m_closeData).movStdDev(period).mul(bandWidth);
		ArrayMath movAvg = ArrayMath(m_closeData).movAvg(period);
		char buffer[1024];
		sprintf(buffer, "%s;Bollinger (%d, %g)", alias.c_str(), period, bandWidth);
		return addBand(ArrayMath(movAvg).add(stdDev), ArrayMath(movAvg).sub(stdDev).selectGTZ(), lineColor,
			fillColor, buffer);
	}

	public: InterLineLayer* addDailyHeight(std::string alias, int lineColor, int fillColor)
	{
		ArrayMath high = computeDailyHigh();
		ArrayMath low = computeDailyLow();
		char buffer[1024];
		sprintf(buffer, "%s;DailyHeight", alias.c_str());
		return addBand(high, low, lineColor,
			fillColor, buffer);
	}

	public: InterLineLayer* addDailyHeight(std::string alias, int highColor, int lowColor, int fillColor)
	{
		ArrayMath high = computeDailyHigh();
		ArrayMath low = computeDailyLow();
		char buffer[1024];
		sprintf(buffer, "%s;DailyHeight", alias.c_str());
		return addBand2(high, low, highColor, lowColor, 0xffffffff, buffer);
	}

	public: InterLineLayer* addEntireHeight(std::string alias, int lineColor, int fillColor)
	{
		ArrayMath high = computeEntireHigh();
		ArrayMath low = computeEntireLow();
		char buffer[1024];
		sprintf(buffer, "%s;EntireHeight", alias.c_str());
		return addBand(high, low, lineColor,
			fillColor, buffer);
	}

	public: InterLineLayer* addEntireHeight(std::string alias, int highColor, int lowColor, int fillColor)
	{
		ArrayMath high = computeEntireHigh();
		ArrayMath low = computeEntireLow();
		char buffer[1024];
	...
	public: InterLineLayer* addPreDailyHeight(std::string alias, int period, int div, int lineColor, int fillColor)
	{
		ArrayMath high = computeDailyHigh(period);
		ArrayMath low = computeDailyLow(period);

		ArrayMath dif(high);
		dif.sub(low).div(4.0);

		char buffer[1024];
		sprintf(buffer, "%s;PreDailyHeight", alias.c_str());

		/*
		for (int i = 0; i < div; i++)
		{
			ArrayMath first = ArrayMath(high).sub(ArrayMath(dif).mul(i + 1));
			m_mainChart->addLineLayer(DoubleArray(first), 0x0ff00, buffer);
		}
		*/
		
		return addBand2(high, low, 0xff000, 0x0000ff,
			fillColor, buffer);
	}

	public: InterLineLayer* addOrbBand(std::string alias, int period, int div, int highColor, int lowColor, int fillColor)
	{
		ArrayMath high = computeDailyHigh(period);
		ArrayMath low = computeDailyLow(period);
		...
		char buffer[1024];
		sprintf(buffer, "%s;OrbBand", alias.c_str());
	...
	public: InterLineLayer* addPreDailyHeight(std::string alias, int period, int div, int highColor, int lowColor, int fillColor)
	{
		ArrayMath high = computeDailyHigh(period);
		ArrayMath low = computeDailyLow(period);
		...
		*/
		...
		return addBand2(high, low, highColor, lowColor,
			0xffffffff, buffer);
	}

	private: ArrayMath computeDailyHigh()
	{
		double *ret = new double[m_highData.len];
		ret[0] = m_highData[0];
		double oldDate = Chart::getChartYMD(m_timeStamps[0]);
		for (int i = 1; i < m_highData.len; ++i)
		{
			double newDate = Chart::getChartYMD(m_timeStamps[i]);
			if (oldDate != newDate)
			{
				ret[i] = m_highData[i];
				oldDate = newDate;
	...
	private: ArrayMath computeDailyLow()
	{
		double *ret = new double[m_lowData.len];
		ret[0] = m_lowData[0];
		double oldDate = Chart::getChartYMD(m_timeStamps[0]);
		for (int i = 1; i < m_lowData.len; ++i)
		{
			double newDate = Chart::getChartYMD(m_timeStamps[i]);
			if (oldDate != newDate)
			{
	...
	private: ArrayMath computeEntireHigh()
	{
		double *ret = new double[m_highData.len];
		ret[0] = m_highData[0];
		for (int i = 1; i < m_highData.len; ++i)
	...
	private: ArrayMath computeEntireLow()
	{
		double *ret = new double[m_lowData.len];
		ret[0] = m_lowData[0];
		for (int i = 1; i < m_lowData.len; ++i)
	...
	private: ArrayMath computeDailyHigh(int period)
	{
		double *ret = new double[m_highData.len];
		double *pre = new double[m_highData.len];
		pre[0] = Chart::NoValue;
		ret[0] = m_highData[0];
		...
		for (int i = 1; i < m_highData.len; ++i)
		{
			...
			if (oldDate != newDate)
			{
				...
				oldDate = newDate;
				count++;
				auto it = dailyHigh.begin();
				dailyHigh.insert(it, ret[i - 1]);
			}
			else
			{
				if (m_highData[i] > ret[i - 1])
			...
			else
			{
				...
				{
					pre[i] = dailyHigh[period - 1];
				}
			}
		}
		ArrayMath ret1(DoubleArray(pre, m_highData.len));
		delete[] ret;
		delete[] pre;
		return ret1;
	}

	private: ArrayMath computeDailyLow(int period)
	{
		double *ret = new double[m_lowData.len];
		double *pre = new double[m_lowData.len];
		ret[0] = m_lowData[0];
		pre[0] = Chart::NoValue;
		...
		for (int i = 1; i < m_lowData.len; ++i)
		{
			...
			if (oldDate != newDate)
			{
				...
				oldDate = newDate;
				count++;
				auto it = dailyLow.begin();
				dailyLow.insert(it, ret[i - 1]);
			}
			else
			{
				if (m_lowData[i] < ret[i - 1])
			...
			else
			{
				if (period != 0 && count >= period)
				{
					pre[i] = dailyLow[period - 1];
				}
			}
		}
		ArrayMath ret1(DoubleArray(pre, m_lowData.len));
		delete[] ret;
		return ret1;
	}


	public: InterLineLayer* addPriceChannel(int period, int lineColor, int fillColor)
	{
		//Bollinger Band is moving avg +/- (width * moving std deviation)
		ArrayMath high = computeHighChannel(period);
		ArrayMath low = computeLowChannel(period);
		char buffer[1024];
		sprintf(buffer, "Price Channel (%d)", period);
		return addBand(high, low, lineColor,
			fillColor, buffer);
	}

	public: InterLineLayer* addPriceChannel2(std::string alias, int period, int lineColor, int fillColor)
	{
		//Bollinger Band is moving avg +/- (width * moving std deviation)
		ArrayMath high = computeHighChannel(period);
		ArrayMath low = computeLowChannel(period);
		...
		char buffer[1024];
		sprintf(buffer, "%s;Price Channel (%d)", alias.c_str(), period);
		return addBand(high, low, lineColor,
			fillColor, buffer);
	}
    /// <summary>
	...
	public: BarLayer* addVolBars3(std::string alias, int height, int upColor, int downColor, int flatColor)
	{
		return addVolBars4(alias, m_mainChart, height, upColor, downColor, flatColor);
	}
    ...
    {       
        ...
        DataSet* upDS = barLayer->addDataSet(ArrayMath(m_volData).selectGTZ(closeChange).result(), upColor, _T("Volume:Up"));
        DataSet* dnDS = barLayer->addDataSet(ArrayMath(m_volData).selectLTZ(closeChange).result(), downColor, _T("Volume:Down"));
        DataSet* flatDS = barLayer->addDataSet(ArrayMath(m_volData).selectEQZ(closeChange).result(), flatColor, _T("Volume:Flat"));
	...
	private: BarLayer* addVolBars4(std::string alias, XYChart* c, int height, int upColor, int downColor, int flatColor)
	{
		BarLayer *barLayer = c->addBarLayer(Chart::Overlay);
		barLayer->setBorderColor(Chart::Transparent);
		...
		std::string label = "Vol";
		if (i >= 0) {
			sprintf(buffer, "%s;%s: %s%s", alias.c_str(), label.c_str(), formatValue(m_volData[i], m_generalFormat.c_str()),
				m_volUnit.c_str());
			label = buffer;
		}

		DataSet* upDS = barLayer->addDataSet(ArrayMath(m_volData).selectGTZ(closeChange).result(), upColor, label.c_str());
		DataSet* dnDS = barLayer->addDataSet(ArrayMath(m_volData).selectLTZ(closeChange).result(), downColor, label.c_str());
		DataSet* flatDS = barLayer->addDataSet(ArrayMath(m_volData).selectEQZ(closeChange).result(), flatColor, label.c_str());
	...
	public: LayerReturn addLineIndicator3(int height, DoubleArray data, int color, const char* name)
	{
		XYChart *c = addIndicator(height);
		LineLayer* layer = addLineIndicator2(c, data, color, name);

		LayerReturn result;
		result.chart = c;
		result.lineLayer = layer;
		return result;
	}
    ...
    /// <returns>The XYChart object representing the chart created.</returns>
    public: XYChart* addATR(int height, int period, int period2, int color1, int color2)
    {
		int startIdx = 0, endIdx = m_lowData.len - 1;
		int outBegIdx = 0, outNbElement = 0;
		double* atr = new double[m_lowData.len];

		int retCode = TA_ATR(startIdx,
			endIdx,
			m_highData.data,
			m_lowData.data,
			m_closeData.data,
		...
		DoubleArray aatr(atr, m_lowData.len);
		ArrayMath amatr(aatr);
		amatr.shift(outBegIdx);

		
        //ArrayMath trueRange = computeTrueRange();
        XYChart *c = addLineIndicator(height, amatr, color1, "True Range");

		DoubleArray inputAtr(amatr);
		double* ema = new double[m_lowData.len];
		retCode = TA_EMA(startIdx,
			endIdx,
			inputAtr.data,
			period2,
			&outBegIdx,
		...
		DoubleArray aema(ema, m_lowData.len);
		ArrayMath aaema(aema);
		aaema.shift(outBegIdx);
        char buffer[1024];
        sprintf(buffer, "Average True Range (%d,%d)", period, period2);
        addLineIndicator2(c, ArrayMath(amatr).expAvg(2.0 / (period + 1)), color2, buffer);
		//addLineIndicator2(c, aaema, color2, buffer);
		delete[] atr;
		delete[] ema;
        return c;
    }

	public: XYChart* addATR2(std::string alias, int height, int period, int period2, int color1, int color2)
	{
		int startIdx = 0, endIdx = m_lowData.len - 1;
		int outBegIdx = 0, outNbElement = 0;
		double* atr = new double[m_lowData.len];
		...
		name.append(_T(";True Range"));
		XYChart *c = addLineIndicator(height, amatr, color1, name.c_str());
		...
		aaema.shift(outBegIdx);
		char buffer[1024];
		sprintf(buffer, "%s;Average True Range (%d,%d)", alias.c_str(), period, period2);
		addLineIndicator2(c, ArrayMath(amatr).expAvg(2.0 / (period + 1)), color2, buffer);
		//addLineIndicator2(c, aaema, color2, buffer);
		delete[] atr;
		delete[] ema;
    ...
    public: XYChart* addFastStochastic(int height, int period1, int period2, int color1, int color2)
    {
        ...
        char buffer[1024];
        sprintf(buffer, "FastStochastic %%K (%d)", period1);
        ...
        int signalColor, int divColor)
    {
        ...
        LineLayer* layer = addLineIndicator2(c, macd, color, buffer);
        ...
        //Divergence
		DoubleArray divergence = ArrayMath(macd).sub(macdSignal);
        addBarIndicator2(c, divergence, divColor, "Divergence");

        return c;
    }

	public: XYChart* addMACD2(std::string alias, int height, int period1, int period2, int period3, int color,
		int signalColor, int divColor)
	{
		XYChart *c = addIndicator(height);
		char buffer[1024];
		...
		//Add the MACD line
		sprintf(buffer, "%s;MACD (%d, %d)", alias.c_str(), period1, period2);
		LineLayer* layer = addLineIndicator2(c, macd, color, buffer);
		layer->setLineWidth(2);
		...
		//MACD signal line
		ArrayMath macdSignal = ArrayMath(macd).expAvg(2.0 / (period3 + 1));
		sprintf(buffer, "%s;EXP (%d)", alias.c_str(), period3);
		addLineIndicator2(c, macdSignal, signalColor, buffer);

		//Divergence
		DoubleArray divergence = ArrayMath(macd).sub(macdSignal);
		std::string name = alias;
		name.append(_T(";Divergence"));
		addBarIndicator2(c, divergence, divColor, name.c_str());
	...
	public: XYChart* addRSI2(std::string alias, int height, int period, int color, double range, int upColor,
		int downColor)
	{
		XYChart *c = addIndicator(height);
		char buffer[1024];
		sprintf(buffer, "%s;RSI (%d)", alias.c_str(), period);
		LineLayer *layer = addLineIndicator2(c, computeRSI(period), color, buffer);
	...
	public: XYChart* addStochastics(int height, int fastK, int slowK, int slowD, int color1, int color2, int range, int upColor, int downColor)
	{
		int startIdx = 0, endIdx = m_lowData.len - 1;
		int outBegIdx = 0, outNbElement = 0;
		double* aSlowK = new double[m_lowData.len];
		double* aSlowD = new double[m_lowData.len];
		...
		DoubleArray daSlowK(aSlowK, m_lowData.len);
		ArrayMath amSlowK(daSlowK);
		DoubleArray daSlowD(aSlowD, m_lowData.len);
		ArrayMath amSlowD(daSlowD);
		amSlowK.shift(outBegIdx);
		amSlowD.shift(outBegIdx);

		char buffer[1024];
		sprintf(buffer, "Slow Stochastic %d %%K (%d)", fastK, slowK);
		LayerReturn result = addLineIndicator3(height, amSlowK, color1, buffer);
		sprintf(buffer, "%%D (%d)", slowD);
		addLineIndicator2(result.chart, amSlowD, color2, buffer);

		LineLayer* layer = result.lineLayer;
		//Add range if given
		if ((range > 0) && (range < 50)) {
			addThreshold(result.chart, layer, 50 + range, upColor, 50 - range, downColor);
		}

		result.chart->yAxis()->setLinearScale(0, 100);
	...
	public: XYChart* addStochastics2(std::string alias, int height, int fastK, int slowK, int slowD, int color1, int color2, int range, int upColor, int downColor)
	{
		int startIdx = 0, endIdx = m_lowData.len - 1;
		int outBegIdx = 0, outNbElement = 0;
		double* aSlowK = new double[m_lowData.len];
		...
		char buffer[1024];
		sprintf(buffer, "%s;Stochastic %d %%K (%d)", alias.c_str(), fastK, slowK);
		LayerReturn result = addLineIndicator3(height, amSlowK, color1, buffer);
		sprintf(buffer, "%s;%%D (%d)", alias.c_str(), slowD);
		addLineIndicator2(result.chart, amSlowD, color2, buffer);
	...
	public: XYChart* addTRIX2(std::string alias, int height, int period, int color)
	{
		double f = 2.0 / (period + 1);
		char buffer[1024];
		sprintf(buffer, "%s;TRIX (%d)", alias.c_str(), period);
		return addLineIndicator(height, ArrayMath(m_closeData).expAvg(f).expAvg(f).expAvg(f)
			.rate().sub(1).mul(100), color, buffer);
	}

	private: ArrayMath computeLowChannel(int len)
	{
		double *ret = new double[m_lowData.len];
		double lowest = m_lowData[0];
		for (int i = 0; i < m_lowData.len; ++i)
		{
			if (i < len - 1)
			{
			...
			{
				lowest = m_lowData[i - len + 1];
				for (int k = i - len + 1; k <= i; ++k)
				{
					if (m_lowData[k] < lowest)
						lowest = m_lowData[k];
				}
				ret[i] = lowest;
			}
	...
	private: ArrayMath computeHighChannel(int len)
	{
		double *ret = new double[m_highData.len];
		double highest = m_highData[0];
		for (int i = 0; i < m_highData.len; ++i)
		{
			if (i < len - 1)
			{
			...
			{
				highest = m_highData[i - len + 1];
				for (int k = i - len + 1; k <= i; ++k)
				{
					if (m_highData[k] > highest)
						highest = m_highData[k];
				}
				ret[i] = highest;
			}
		}
...
public:
	...
	void BarWidth(int val) { m_BarWidth = val; }
	int LeftMargin() const { return m_leftMargin; }
	void LeftMargin(int val) { m_leftMargin = val; }
	int RightMargin() const { return m_rightMargin; }
	void RightMargin(int val) { m_rightMargin = val; }

	std::map<std::string, DoubleArray> IndexDataMap;

	XYChart * MainChart() const { return m_mainChart; }
	void MainChart(XYChart * val) { m_mainChart = val; }
};
