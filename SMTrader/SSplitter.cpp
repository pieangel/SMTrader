...
CSSplitter::CSSplitter()
{
	...
	m_nSplitterWidth			= 2;   // the width of slitter
...
void CSSplitter::HideSibling()
{
	if (Sibling)
	{
		CSSplitter* sible = Sibling;
...
void CSSplitter::OnSize(UINT nType, int cx, int cy) 
{
	...
	*/
	...
	if (m_bHorizSplitter && m_bHiddenTopPane && (m_pWndBottom->GetSafeHwnd())) {
		m_pWndBottom->MoveWindow(&rect);
	}
	
	if ( m_bHorizSplitter && (!m_bHiddenBottomPane) && (!m_bHiddenTopPane)){ 
		...
		UINT	nMaxRight)
{
	...
	*/
	/*
	int nLeftPaneWidth = rect.Width() / 2;
	...
	*/
	m_nLeftPaneWidth = rect.Width() / 2;
	CPoint InitPoint(m_nLeftPaneWidth + m_nSplitterWidth / 2, 0);
	MoveVertPanes(InitPoint);
}
		...
		UINT	nMaxBottom)
{
	...
	*/
	m_nBottomPaneHeight = rect.Height() / 2;
	...
	if (!m_bHiddenTopPane) {
		CPoint InitPoint(m_nBottomPaneHeight + m_nSplitterWidth / 2, 0);
		MoveHorizPanes(InitPoint);
	}

	if ((m_bHiddenTopPane) && (m_pWndTop->GetSafeHwnd())) {
...
void CSSplitter::ShowTopPane()
{
	...
	if (!m_rcWndTop.IsRectEmpty())
		m_nBottomPaneHeight = m_rcWndTop.Height();
...
BOOL CSSplitter::Create(DWORD dwStyle, CWnd* pParentWnd, CWnd* pFPane, CWnd* pSPane, UINT nID, const RECT& rc, UINT nFConstr /*= 30*/, UINT nSConstr /*= 30 */)
{
	m_nID  = nID;
	CStatic::Create(NULL, dwStyle, rc, pParentWnd, nID);
	...
	if (SplitType == VtSplitType::SplitVertical)
	{
		InitVertSplitter(
			this,
			pFPane,
	...
	if (SplitType == VtSplitType::SplitHorizontal)
	{
		InitHorizSplitter(
			this,
			pFPane,
