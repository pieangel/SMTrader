...
VtKp3s::VtKp3s(VtSystemType type)
	:VtSystem(type)
{
	InitArgs();
}

VtKp3s::VtKp3s(VtSystemType systemType, std::string name)
...
void VtKp3s::CreateSignal(int startIndex, int endIndex)
{
	if (!_DateTime)
		return;
...
/// <returns></returns>
VtPositionType VtKp3s::UpdateSignal(int index)
{
	// 시스템 업데이트
	UpdateSystem(index);
...
VtPositionType VtKp3s::UpdateSignal(int start, int end)
{
...
void VtKp3s::SetDataMap(VtChartData* chartData)
{
...
void VtKp3s::SaveToXml(pugi::xml_node& node)
{
...
void VtKp3s::LoadFromXml(pugi::xml_node& node)
{
...
void VtKp3s::UpdateSystem(int index)
{
...
bool VtKp3s::CheckEntranceForBuy()
{
	// 밴드에 의한 조건을 먼저 확인한다.
...
bool VtKp3s::CheckEntranceForSell()
{
	// 밴드에 의한 조건을 먼저 확인한다.
...
bool VtKp3s::CheckEntranceByBandForBuy()
{
	if (!_Symbol)
		return false;
...
bool VtKp3s::CheckEntranceByBandForBuy(size_t index)
{
...
bool VtKp3s::CheckEntranceByBandForSell()
{
	if (!_Symbol)
		return false;
...
bool VtKp3s::CheckEntranceByBandForSell(size_t index)
{
	if (!_Symbol || index < 0 || index >= ChartDataSize)
		return false;
...
bool VtKp3s::CheckEntranceByOpenForBuy()
{
	if (!_Symbol)
		return false;
...
bool VtKp3s::CheckEntranceByOpenForBuy(size_t index)
{
	if (!_Symbol || index < 0 || index >= ChartDataSize)
		return false;
...
bool VtKp3s::CheckEntranceByOpenForSell()
{
	if (!_Symbol)
		return false;
...
bool VtKp3s::CheckEntranceByOpenForSell(size_t index)
{
	if (!_Symbol || index < 0 || index >= ChartDataSize)
		return false;
...
bool VtKp3s::CheckEntranceForBuy(size_t index)
{
...
bool VtKp3s::CheckEntranceForSell(size_t index)
{
	if (index < 0 || index >= ChartDataSize)
		return false;
...
bool VtKp3s::CheckLiqForSell(size_t index)
{
	if (index < 0 || index >= ChartDataSize)
		return false;
...
bool VtKp3s::CheckLiqForBuy(size_t index)
{
	if (index < 0 || index >= ChartDataSize)
		return false;
...
void VtKp3s::ReloadSystem(int startIndex, int endIndex)
{
	ClearSignal();
