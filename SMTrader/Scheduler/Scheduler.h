...
namespace Bosma {
    ...
    public:
		...
		Clock::time_point get_timepoint(const std::string& input_time) {
			...
			if (try_parse(tm, input_time, "%H:%M:%S")) {
				...
				throw std::runtime_error("Cannot parse time string: " + input_time);
			}
        ...
        template<typename _Callable, typename... _Args>
		std::chrono::system_clock::time_point at(const std::string &time, _Callable &&f, _Args &&... args) {
          // get current time as a tm object
          auto time_now = Clock::to_time_t(Clock::now());
          std::tm tm = *std::localtime(&time_now);
...
//    * * * * *
		...
		void remove_task(std::chrono::system_clock::time_point tp) {
