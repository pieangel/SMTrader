...
void CGridCtrl::DrawStopOrders(CDC* pDC)
{
	if (_StopOrderVector.size() > 0) {
		for (size_t i = 0; i < _StopOrderVector.size(); ++i) {
			std::pair<CCellID, CCellID> item = _StopOrderVector[i];
			CPoint pt1, pt2;
			CRect startRect, endRect;
			GetCellRect(item.first, startRect);
			GetCellRect(item.second, endRect);
			...
			if (item.first.col == 0)
				DrawArrow(1, pDC, pt1, pt2, 6, 6);
			else
				DrawArrow(2, pDC, pt1, pt2, 6, 6);
		}
	}
}

void CGridCtrl::DrawCloseLine(CDC* pdc)
{
	...
	CPoint pt1, pt2;
	CRect startRect, endRect;
	GetCellRect(_CloseLineRow, 0, startRect);
	GetCellRect(_CloseLineRow, 8, endRect);
	pt1.x = startRect.left;
	pt1.y = startRect.bottom;
	pt2.x = endRect.right;
	pt2.y = endRect.bottom;
